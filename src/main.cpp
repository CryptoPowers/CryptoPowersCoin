...
uint256 hashGenesisBlock("0x00000e3d1522ffe9503343bfcbeb65a8e2bf45877f84a1159f15fac10d05f92d"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CryptoPowers: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CryptoPowers Signed Message:\n";
...
{
    // CryptoPowers: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 nSubsidy = 0 * COIN;
    ...
    if(nHeight == 1) {
       nSubsidy = 14185074 * COIN;
    } else {
        nSubsidy = 10000 * COIN;
    }
    
	// Susidy is cut in half every 1440 block, Which is every day.
	nSubsidy >>= (nHeight / 1440); 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60 * 5; // CryptoPowers: Every 5 Hours
static const int64 nTargetSpacing =  60; // CryptoPowers: 1 Minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CryptoPowers: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, CryptoPowers - DarkGravity v2, written by Evan Duffield - evan@CryptoPowers.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
{
        static const int64 BlocksTargetSpacing = 1 * 60; // 1 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CryptoPowers: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // CryptoPowers: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0000085aaf5b2a0ae49644e6ed2df2976d2f564ef68e73b0d5a11ea381db0ffa");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "April 28 2014 Malaysian flight 370 still missing...";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1398699093;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 15662702;

        if (fTestNet)
        {
            block.nTime    = 1398699129;
            block.nNonce   = 1371518;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xa1b9e9a411f672a96048b80df558c2c1aca028ce3e024e2806587f9a8d2fa33f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf6, 0xc6, 0xb6, 0xd6 }; // CryptoPowers: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryptoPowersMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryptoPowersMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CryptoPowersMiner(CWallet *pwallet)
{
    printf("CryptoPowersMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("CryptoPowers-miner");
    ...
    try { loop {
        ...
        printf("Running CryptoPowersMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CryptoPowersMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CryptoPowersMiner, pwallet));
}
